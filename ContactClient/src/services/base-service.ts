...
export class BaseService<TEntity extends IBaseEntity> {
  ...
  fetchAll(): Promise<TEntity[]> {
    ...
    return this.serviceHttpClient.fetch(url, {cache: 'no-store'})
      .then(response => {
  ...
  post(entity: TEntity): Promise<Response> {
    ...
    return this.serviceHttpClient.post(url, JSON.stringify(entity), {cache: 'no-store'}).then(
      response => {
  ...
  put(entity: TEntity): Promise<Response> {
    ...
    return this.serviceHttpClient.put(url, JSON.stringify(entity), {cache: 'no-store'}).then(
      response => {
        log.debug('response', response);
        return response;
      }
  ...
  delete(id: number): Promise<Response> {
    ...
    return this.serviceHttpClient.delete(url, {cache: 'no-store'}).then(
      response => {
        log.debug('response', response);
        return response;
      }
